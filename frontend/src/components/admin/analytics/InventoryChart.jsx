        ...
        <div className="bg-card border border-black/5 dark:border-white/5 rounded-xl shadow-sm overflow-hidden h-full flex flex-col">
            ...
            <div className="flex-grow p-4 min-h-[300px]">
                {data.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                            ...
                            <XAxis
                                ...
                                dy={10}
                                tickFormatter={(tick) => tick.length > 10 ? `${tick.substring(0, 8)}.` : tick}
                            />
                            <YAxis
                                axisLine={false}
                                tickLine={false}
                    ...
                    <div className="flex h-full items-center justify-center text-text-muted text-sm">No inventory data</div>
                )}
            </div>
        </div>
    );
